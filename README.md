...
- **Google 계정 문서 동기화/보내기/받기** — Firebase Auth 로그인 후 내 문서를 Firestore에 동기화해 다른 기기에서도 불러오고, 현재 MD 문서 사본을 다른 Google 이메일로 전송하고, 받은 문서함에서 내 문서로 가져오기
...
11. 상단의 Google 로그인 후 내 문서는 자동으로 클라우드에 동기화됩니다. 다른 컴퓨터나 모바일에서도 같은 Google 계정으로 로그인하면 내 문서 목록을 불러오며, 로그아웃하면 계정 문서는 화면에서 숨겨집니다.
...
**데이터 보관**
- 로그아웃 상태의 문서는 브라우저의 IndexedDB에 저장되며, 같은 기기·브라우저에서만 접근할 수 있습니다. Google 로그인 시 기존 로컬 문서는 내 계정의 Firestore 문서로 업로드되고, 이후 로그인 상태의 편집 내용은 클라우드 문서에 자동 저장됩니다.
