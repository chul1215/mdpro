...
- **다중 문서·폴더 관리** — IndexedDB 기반 자동 저장, 문서 목록/생성/삭제/전환, 폴더별 분류와 이동
- **내보내기·가져오기** — `.md`(Blob 다운로드), `.html`(완전 독립형 문서, Mermaid SVG 인라인), 드래그 앤 드롭 또는 파일 선택
- **Google 계정 문서 보내기/받기** — Firebase Auth 로그인 후 현재 MD 문서 사본을 다른 Google 이메일로 전송하고, 받은 문서함에서 내 로컬 문서로 가져오기
...
7. 사이드바 문서 탭의 **폴더** 버튼으로 분류 폴더를 만들고, 문서 행의 **폴더 이동** 선택창으로 문서를 옮길 수 있습니다.
8. **보안** 버튼으로 암호코드가 필요한 폴더를 만들 수 있습니다. 잠긴 폴더는 암호코드 입력 후 열립니다.
9. 파일 메뉴(상단 우측 아이콘)에서 `.md`/`.html` 내보내기, `.md` 가져오기를 할 수 있습니다.
10. 마크다운 파일을 창에 드래그 앤 드롭하면 새 문서로 가져옵니다.
11. 상단의 Google 로그인 후 **문서 보내기** 버튼으로 현재 문서 사본을 다른 사용자 이메일에 보낼 수 있습니다.
12. 사이드바 **받은함** 탭에서 받은 문서를 확인하고 **내 문서로 가져오기**를 누르면 IndexedDB 문서 목록에 복사됩니다.
13. 사이드바 **주소록** 탭에서 자주 보내는 사람을 저장하거나, 문서 보내기 화면에서 **주소록에 저장**을 체크해 자동 추가할 수 있습니다.

**데이터 보관**
- 모든 문서는 브라우저의 IndexedDB에 저장되며, 같은 기기·브라우저에서만 접근할 수 있습니다.
- 폴더 정보와 보안 폴더 암호코드 해시는 현재 브라우저의 localStorage에 저장됩니다. 보안 폴더는 UI 잠금 기능이며, 기기 자체 보안과 내보내기 백업 관리를 함께 사용하세요.
