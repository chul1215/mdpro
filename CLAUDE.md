...
### 전체 프로젝트 현황 (최종 갱신 2026-05-18 · 브랜딩 변경 + Cloudflare Pages 라이브)

- **완료 페이즈**: Phase 0 (스캐폴딩) · Phase 1 (레이아웃) · Phase 2 (CodeMirror 에디터) · Phase 3 (unified 프리뷰 파이프라인) · Phase 4 (서식 툴바) · Phase 5 (IndexedDB 다중 문서) · Phase 6 (.md/.html 내보내기 + .md 가져오기) · Phase 7 (E2E 확대 · 접근성 자동 감사 · README/LICENSE · Cloudflare Pages 배포 설정 · 수동 검증 체크리스트) · **포스트-릴리스: Apple 디자인 시스템**(SF Pro + 단일 강조색 + 글래스 TopBar) · **포스트-릴리스: 브랜딩 변경**(MDPro → MD Practice (SMC AI실무도입전환 프로젝트), 기술 식별자는 호환 위해 유지) · **포스트-릴리스: 첫 프로덕션 배포**(Cloudflare Pages, https://md-practice.pages.dev)
- **작업 트리 상태**: `main` == `origin/main`(721b9be). 워크트리에 `.github/workflows/deploy.yml`(미커밋 — GitHub Secret 등록 후 푸시 예정)과 `.claude/scheduled_tasks.lock`(로컬 부산물) untracked. 기능 로드맵은 Phase 7까지 확정되어 있고, 이후는 디자인/유지보수성 개선 + 운영 커밋으로 누적된다.
...
- **직전 작업**: 브랜딩 변경 + 첫 프로덕션 배포(`721b9be`) — UI/문서/E2E 노출 명칭을 'MDPro' → 'MD Practice (SMC AI실무도입전환 프로젝트)'로 일괄 교체. TopBar는 메인 'MD Practice' + 부제(SMC AI실무도입전환 프로젝트) 2단 구조, 부제는 `text-white/70`로 글래스 배경(`rgba(0,0,0,0.72)`) 위 AA 충족, md 미만에서는 숨김. `index.html` title 동기화. README/CLAUDE.md/.claude/agents/*.md 4종 사용자 노출 멘션 갱신. **기술 식별자(IndexedDB DB `mdpro`, localStorage `mdpro-ui`/`mdpro-doc`, `package.json` name, `MDProDB` 타입)는 기존 사용자 문서 호환을 위해 의도적으로 유지**. E2E 4개 스펙의 heading name 매칭 동기화. `.gitignore`에 `.playwright-browsers/` 추가. Wrangler CLI(`npx wrangler@latest`)로 Cloudflare Pages 프로젝트 `md-practice` 생성 + `dist` 직접 업로드 → https://md-practice.pages.dev 라이브. 직후 GitHub Actions 자동 배포 워크플로우 `.github/workflows/deploy.yml` 작성(미커밋, Secret 등록 대기).
- **검증 결과**: `npm run check` 175/175, `npm run test:e2e` 8/8 (axe WCAG 2.1 AA 위반 0), `npm run build` 성공(메인 **245.88KB gzipped**, 브랜딩 부제 텍스트분 +0.07KB), 프로덕션 URL HTTP 200, `<title>` 신규 명칭 반영, 임의 경로 → 200(SPA fallback 정상).
...
- **원격 저장소**: https://github.com/chul1215/mdpro (public, `main` → `origin/main`). 최초 푸시 커밋 `e2e5dd6`, 최신 `721b9be`(브랜딩 변경).
- **라이브 사이트**: https://md-practice.pages.dev (Cloudflare Pages, Account `371ac48db3a6148c7eb73680b0ffad1c` 소유). 현재는 Wrangler CLI Direct Upload 모드 — GitHub Actions 자동 배포 활성화는 사용자 Secret 등록 대기 중.
...
> 직전 작업: 브랜딩 변경(`721b9be`) 후 `npm run check` 175/175, `npm run test:e2e` 8/8 (axe AA 위반 0), `npm run build` 메인 245.88KB gzipped. Cloudflare Pages 첫 배포(https://md-practice.pages.dev) HTTP 200 + SPA fallback 정상.
> 다음 작업: 수동 재배포는 `npm run build && npx wrangler@latest pages deploy dist --project-name=md-practice --branch=main --commit-dirty=true`. GitHub Actions 자동 배포 워크플로우(`.github/workflows/deploy.yml`)는 작성 완료 — 사용자가 `CLOUDFLARE_API_TOKEN`(Cloudflare Pages: Edit 권한) + `CLOUDFLARE_ACCOUNT_ID`(`371ac48db3a6148c7eb73680b0ffad1c`) 두 Secret을 등록한 뒤 워크플로우 파일을 커밋·푸시하면 자동 배포로 전환된다. E2E 환경: macOS Library 캐시 root 소유 회피용 `PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers"` 환경변수 필요.
...
- **라이브 URL**: https://md-practice.pages.dev (프로덕션). 배포 단위별 영구 링크는 `https://<해시>.md-practice.pages.dev`. Cloudflare Account `371ac48db3a6148c7eb73680b0ffad1c` 소유, 이메일 `chul871215@gmail.com`.
- **프로젝트 종류**: 현재 "Direct Upload" 모드(Wrangler CLI로 생성). Cloudflare Pages는 Direct Upload 프로젝트를 Git 연동으로 직접 전환할 수 없으므로, 자동화는 GitHub Actions 경로로 진행한다(아래).
- **수동 재배포 명령**: `npm run build && npx wrangler@latest pages deploy dist --project-name=md-practice --branch=main --commit-dirty=true`
- **자동 배포 워크플로우(Secret 등록 후 활성화)**: `.github/workflows/deploy.yml` — 트리거 `push: branches: [main]` + `workflow_dispatch`. 단계: `actions/checkout@v4` → `actions/setup-node@v4`(Node 20, npm cache) → `npm ci` → `npm run build` → `cloudflare/wrangler-action@v3`로 `pages deploy dist --project-name=md-practice --branch=main`. 필요한 GitHub Secret: `CLOUDFLARE_API_TOKEN`(Custom token, Permissions: Account → Cloudflare Pages → Edit), `CLOUDFLARE_ACCOUNT_ID`(`371ac48db3a6148c7eb73680b0ffad1c`). 두 Secret 등록 후 워크플로우 파일을 커밋·푸시하면 첫 자동 빌드가 트리거된다.
- `public/_redirects`에 `/*  /index.html  200` — SPA fallback. 빌드 시 `dist/`로 자동 복사된다.
- 빌드 설정 기준값: Framework `Vite`, Build command `npm run build`, Output `dist`, Node 20+.
- IndexedDB(`mdpro`)는 origin 격리 — 동일 도메인에서만 데이터 공유. 즉 `md-practice.pages.dev` 사용자 문서는 다른 도메인/계정과 섞이지 않는다. 커스텀 도메인을 추가하면 origin이 달라지므로 데이터도 분리됨에 유의.
- **Wrangler 인증**: 로컬에서 `npx wrangler@latest login` → 브라우저 OAuth → 토큰을 `~/Library/Preferences/.wrangler/` 등에 저장. 인증 만료 시 동일 명령으로 재인증. CI 환경(GitHub Actions)에서는 OAuth 대신 API 토큰을 환경변수로 주입.
- **로컬 Playwright 캐시 회피**: macOS `~/Library/Caches/ms-playwright/`가 root 소유라 표준 경로 설치가 막혀 있는 환경에서는 `PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers" npx playwright install chromium`으로 로컬 경로에 설치한 뒤 동일 환경변수로 `npm run test:e2e` 실행. `.playwright-browsers/`는 `.gitignore`로 제외됨. 항구적 해결은 `sudo chown -R $USER:staff ~/Library/Caches/ms-playwright` 후 재설치.
...
> 직전 작업: 브랜딩(`721b9be`) — TopBar `<h1>MDPro</h1>` 단일 텍스트를 메인 'MD Practice' + 부제(`text-white/70`, md 이상에서만 노출, 글래스 배경 위 AA 충족) 2단으로 교체. 기술 식별자(`MDProDB` 타입, IDB `mdpro`, localStorage `mdpro-ui`/`mdpro-doc`)는 그대로 유지해 기존 사용자 데이터 호환. Wrangler CLI로 Cloudflare Pages `md-practice` 프로젝트 생성·라이브.
> 다음 작업: 사용자 노출 텍스트 변경 시 (1) `index.html` `<title>` (2) TopBar heading의 accessible name (3) E2E heading name 매칭 4곳까지 함께 동기화 필수. 디자인 토큰 규칙 변동 없음 — `blue-500` + `apple.*` + `surface.*`, 텍스트 대비 `/70` 기준, `backdrop-filter` 컨테이너는 내부 팝오버 z-index 검증. 향후 사용자 인증/문서 동기화 백엔드 도입 시 IndexedDB 어댑터 레이어와의 경계를 먼저 설계할 것.
...
> 직전 작업: 브랜딩 변경 + 첫 프로덕션 배포 — 단위 175/175, E2E 8/8, axe WCAG 2.1 AA 위반 0. 빌드 메인 245.88KB gzipped(+0.07KB). 라이브: https://md-practice.pages.dev.
> 다음 작업: 릴리스 후보마다 `docs/manual-checklist.md` 10개 영역 수행 + 라이브 URL에서 SPA fallback / IndexedDB 보존 / 다크모드 / 모바일 동작 스팟체크. GitHub Actions 자동 배포 활성화 후 첫 자동 빌드 성공 여부를 릴리스 노트에 기록.
...
> 직전 작업: 브랜딩 변경 + Cloudflare Pages 배포도 메인 에이전트 단독 처리. 14개 파일 텍스트 치환 + Wrangler CLI 호출이라 분산할 만한 작업 영역이 없었음. 사용자 OAuth 단계(`wrangler login`)와 GitHub Secret 등록(`CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID`)은 보안상 사용자 직접 작업으로 안내.
> 다음 작업: 신규 페이즈/기능 시 시그니처(타입/함수명)를 사전에 고정해 3~4개 서브에이전트를 병렬 실행. 사용자 인증 + 문서 동기화 백엔드 같은 큰 작업이 들어오면 `markdown-architect`로 모듈 경계와 IndexedDB 어댑터 추상화 먼저 잡고 그 뒤 `export-specialist`/`ui-designer` 분산.
