...
- [ ] Firebase 환경 변수 미설정 상태에서도 기존 로컬 편집/저장 기능이 정상 동작하는지
- [ ] Google 로그인 버튼 클릭 시 Google 계정 선택 팝업이 열리는지
- [ ] 로그인 후 계정 이름/이메일이 TopBar에 표시되고 로그아웃이 가능한지
- [ ] 문서 보내기: 받는 사람 이메일 입력 → Firestore `shares` 문서 생성
- [ ] 잘못된 이메일 입력 시 inline error가 표시되고 Firestore write가 발생하지 않는지
- [ ] 받은함: 수신자 계정으로 로그인 시 받은 문서 목록이 표시되는지
- [ ] 받은 문서 **내 문서로 가져오기** 후 IndexedDB 문서 목록에 새 문서가 생성되고 활성화되는지
- [ ] 주소록 탭에서 이름/이메일 추가, 중복 이메일 갱신, 삭제가 정상 동작하는지
- [ ] 문서 보내기 화면에서 주소록 연락처 선택 시 받는 사람 이름/이메일이 채워지는지
- [ ] 문서 보내기 성공 후 **주소록에 저장** 체크 시 연락처가 저장되는지
- [ ] Firestore Rules: 비로그인 사용자, 제3자 이메일 사용자가 share를 읽거나 수정할 수 없는지
- [ ] 민감 문서 공유 전 경고/설명 문구가 사용자에게 충분히 보이는지
